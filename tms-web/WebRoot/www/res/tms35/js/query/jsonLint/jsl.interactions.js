    ...
    **/
    function validate(jqObj) {
        ...
        try {
            ...
            if (result) {
				$('#results').text('');
				$('#results').removeClass('error').hide();

                if (reformat) {
                    jqObj.val(JSON.stringify(JSON.parse(jsonVal), null, "    "));
                }
            ...
            **/
            try {
                if (reformat) {
                    jsonVal = jsl.format.formatJson(jqObj.val());
                    jqObj.val(jsonVal);
                    result = jsl.parser.parse(jqObj.val());
                }
            } catch(e) {
                parseException = e;
            }
            ...
            if (lineMatches && typeof lineMatches === "object" && lineMatches.length > 1) {
                ...
                jqObj.focus().caret(lineStart, lineEnd);
            }
