...

{
	...
	if(objStr==null||objStr=='null'||objStr==''||objStr==undefined||objStr=='undefined'||Trim(objStr)=='')
    {
        return false;
    }
    else
...
function IsNumber(objStr,sign,deci)
{
  ...
  var isnum = regu.test(objStr);
  
  // 校验001,00.1类型的数字
  if (isnum) {
  	if (sign == "-") {
  		if (objStr.length >= 3 && objStr.substring(1, 2) == 0 && IsInt(objStr.substring(2, 3))) {
  			isnum = false;
  		}
  	}
  	else {
  		if (objStr.length >= 2 && objStr.substring(0, 1) == 0 && IsInt(objStr.substring(1, 2))) {
  			isnum = false;
  		}
  	}
  }
...
//验证是否含有xml特殊字符
function checkXmlSpecialCode(str){
//	var reg = "~`,!#$￥%^&*{}][【】《》><?？/\\;'；！";	//特殊字符
	var reg = "&><'\"";	//特殊字符（描述信息可能会有中英文分号、叹号）
	for(var j=0;j<reg.length;j++){
		if(str.indexOf(reg.charAt(j))!=-1){
...
//条件验证是否含有特殊字符
function checkeCondSpecialCode(str){
	// 条件的特殊字符校验通过后台校验
	/*var reg = "~`$￥^&【】《》?？\\'";
	for(var j=0;j<reg.length;j++){
		if(str.indexOf(reg.charAt(j))!=-1){
 ...
 */
function checkSpecialCharacter(src, type) { 
	return type == '1' 
		? /^[a-zA-Z][\w]*$/.test(src) 
		: /^[\w\u4e00-\u9fa5]*$/.test(src);
}
 ...
 * return 带有交易属性的提示
 */
function checkSpecialCharacterInTxn(desc, src, type) {
