...
(function() {
	...
	model.rule_c = {
		...
		load: function(ruleListDialog,txnId,type,st_id) {
			txn_id = txnId;
		...
		initFlag: function(ruleListDialog,txnId, type,st_id, page){// type 0:规则，1：策略规则
			txn_id = txnId;
			...
			grid = new jcl.ui.Grid({
				...
				qform:{display:false,items:[
	              ...
	              {name:'DISPOSAL', label:'处置方式', type:'selector', title:'--请选择--', title:'--请选择--', items:[{text:'--请选择--',value:''}], ds:{type:'remote', url:'/tms35/rule/disposal', parser:{key:'row', text:'DP_NAME', value:'DP_CODE'}, lazyLoad:true}, selectedTopItem:true},
				...
				table:{
					...
					rowForm:{
		                items:[
	                       ...
	                       {label:"处置方式", name: 'DISPOSAL', type:'selector', items:[{text:'--请选择--',value:''}], required:true},
			...
			grid.table.rowForm.jqDom.on('dblclick', '[name=RULE_COND_IN]', function(){
				_ce.init_cond(grid.table.rowForm.getItem('RULE_COND').component,$(this),txn_id, ['RULE_FUNC','AC_FUNC'],'条件');
			});
			...
			// 加载数据
			this.load(ruleListDialog,txn_id,type,st_id); 
			
			//ruleListDialog.addDom('rule_ddddd');
		}
	...
	function check_rule_data(data){
		...
		if (!checkeLength(rule_cond, 512)) {
			alert('规则条件不能超过512个字符');
			return false;
		}
		if(Trim(eval_type) == "") {
			alert('评估类型不能为空');
