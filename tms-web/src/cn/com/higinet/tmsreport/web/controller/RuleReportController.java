...
public class RuleReportController {
	...
	public void exportListAction(@RequestParam Map<String, String> reqs,HttpServletResponse response){
		decodeForRequestParamMap(reqs, new String[] {"ruleName"}, null);
		List<Map<String, Object>>  ruleList = ruleReportService.exportList(reqs);

		String titles [] = {"规则名称","监控交易名称","交易执行次数","规则命中数","规则命中率(%)","交易执行占比(%)","命中占比(%)"};
		String colum [] = {"RULENAME","TXNNAME","TRIGGERNUM","HITNUM","HITRATE",
							"TRIGGERRATE","HITNUMRATE"};
		try{
