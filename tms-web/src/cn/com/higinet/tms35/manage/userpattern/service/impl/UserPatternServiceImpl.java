 ...
 * All right reserved.
 */
package cn.com.higinet.tms35.manage.userpattern.service.impl;
...
public class UserPatternServiceImpl implements UserPatternService {
	...
	public List<Map<String, Object>> queryStatPatternList(Map<String, Object> reqs) {
		...
		Map<String, Object> statInfo = statService.getOneStat(statId);

		String stat_fn = MapUtil.getString(statInfo, DBConstant.TMS_COM_STAT_STAT_FN);
		String stat_fn_p = MapUtil.getString(statInfo, DBConstant.TMS_COM_STAT_STAT_FN_PARAM);
		String stat_txn = MapUtil.getString(statInfo, DBConstant.TMS_COM_STAT_STAT_TXN);
		String stat_name = MapUtil.getString(statInfo, DBConstant.TMS_COM_STAT_STAT_NAME);

		String sql = "  select STAT_VALUE,STAT_VALUE2,STAT_LOB  from TMS_RUN_STAT where STAT_PARAM = ?";

		List<Map<String, Object>> statList = tmsSimpleDao.queryForList(sql, userId + "-");

		List<Map<String, Object>> re_sta_list = new ArrayList<Map<String, Object>>();

		if (statList != null && statList.size() > 0) {
			String d = PropertiesUtil.getPropInstance().get("tms.stat.base64");
			stat_number_encode.setM_codec(d == null ? 0 : Integer.parseInt(d));
			stat_value sv = new stat_value(userId + "-", MapUtil.getString(statList.get(0), "STAT_VALUE") + MapUtil.getString(statList.get(0), "STAT_VALUE2") + MapUtil.getString(statList.get(0), "STAT_LOB"));
			String m = sv.get(Integer.valueOf(statId));

			stat_func func = stat_func.get(stat_fn);
			cache_init.init(new data_source(officialTransactionManager.getDataSource()));

			db_stat stat = db_cache.get().stat().get(stat_txn, stat_name);

			Object r = func.getAll(m, stat, (int) (date_tool.parse(date_tool.now()).getTime() / 1000 / 60));

			if (r != null) {
				if (r instanceof Map) {
					Map<String, Object> rm = (Map<String, Object>) r;
					double c = 0;
					Set<Entry<String, Object>> sn = rm.entrySet();
					for (Entry<String, Object> en : sn) {
						c += Double.parseDouble(String.valueOf(en.getValue()));
					}
					DecimalFormat df = new DecimalFormat("0.00");
					for (Entry<String, Object> en : sn) {
						Map<String, Object> a = new HashMap<String, Object>();
						String stat_value = en.getKey();
						if (stat_fn.equals("rang_bin_dist")) {
							String[] split = stat_fn_p.split("\\|");
							if (Integer.parseInt(stat_value) < split.length) {
								stat_value = split[Integer.parseInt(stat_value)];
							}
						} else {
							// String txnSql = tmsSqlMap.getSql("tms.userpattern.queryTxnFeature");
							String txnSql = "SELECT CODE_KEY,CODE_VALUE,TYPE,CODE FD_CODE FROM ( SELECT REF_NAME CODE_KEY,NAME CODE_VALUE,TYPE,CODE,TAB_NAME FROM TMS_COM_FD UNION SELECT REF_NAME CODE_KEY,REF_DESC CODE_VALUE,'' TYPE,'' CODE,TAB_NAME FROM TMS_COM_REFFD ) F WHERE F.TAB_NAME IN ("
									+ TransCommon.arr2str(TransCommon.cutToIds(stat_txn)) + ") AND CODE_KEY = ? ORDER BY TAB_NAME";

							String txnFeature = MapUtil.getString(statInfo, DBConstant.TMS_COM_STAT_STAT_DATAFD);
							// 查询交易属性
							List<Map<String, Object>> l = tmsSimpleDao.queryForList(txnSql, txnFeature);

							stat_value = code2Text(stat_value, txnFeature, MapUtil.getString(l.get(0), "CODE"));
						}

						a.put("STAT_VALUE", stat_value);
						a.put("STAT_COUNT", en.getValue());
						a.put("STAT_PERCENT", c == 0 ? 0 : df.format(Double.parseDouble(String.valueOf(en.getValue())) * 100 / c) + "%");
						re_sta_list.add(a);
					}
				} else {

					// 快照的日期时间类型需要把毫秒数转成日期格式显示
					if (stat_fn.equals("snapshot")) {
						// String txnSql = tmsSqlMap.getSql("tms.userpattern.queryTxnFeature");
						String txnSql = "SELECT CODE_KEY,CODE_VALUE,TYPE,CODE FD_CODE FROM ( SELECT REF_NAME CODE_KEY,NAME CODE_VALUE,TYPE,CODE,TAB_NAME FROM TMS_COM_FD UNION SELECT REF_NAME CODE_KEY,REF_DESC CODE_VALUE,'' TYPE,'' CODE,TAB_NAME FROM TMS_COM_REFFD ) F WHERE F.TAB_NAME IN ("
								+ TransCommon.arr2str(TransCommon.cutToIds(stat_txn)) + ") AND CODE_KEY = ? ORDER BY TAB_NAME";
						...
						if (l != null && l.size() > 0) {
							String feature_type = MapUtil.getString(l.get(0), "TYPE");
							if (feature_type.equals("datetime")) {
								Format f = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
								r = f.format(new Date((Long) r));
							}
						}
						r = code2Text(String.valueOf(r), txnFeature, MapUtil.getString(l.get(0), "CODE"));
					}
