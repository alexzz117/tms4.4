...
public class MonitorStatServiceImpl implements MonitorStatService {
	...
	public void updateMonitorStat(Map<String, Object> traffic, String status) {
		...
		Map<String, Object> txnAreaConds = getTxnAreaStatCondsMap(traffic,
				txnConds);// 组织带地区交易统计的查询条件Map
		...
		if (StringUtil.isBlank(disposal) || "null".equals(disposal) || "PS01".equals(disposal))// 无风险
		{
			...
			if (StringUtil.isBlank(correct) || "null".equals(correct))
				return;
		...
		if (MapUtil.isEmpty(txnAreaStatMap)) {
			...
			monitorStatReCalculate(txnAreaStatMap, modelrisk_authpass_confirmrisk_number,
					modelrisk_authpass_confirmnorisk_number, modelrisk_authnotpass_confirmrisk_number,
	 ...
	 * @param arg5
	 */
	private void monitorStatReCalculate(Map<String, Object> statMap, long arg0,
	...
	private Map<String, Object> getTxnAreaStatCondsMap(
			Map<String, Object> traffic, Map<String, Object> conds) {
	...
	private long getMonitorStatTime(long txntime) {
		String sql = "select s.TIME from TMS_MONITOR_TXN_AREA_STAT s "
				+ "where s.TIME <= " + txntime + " order by s.TIME desc";
	...
	@Override
	@Transactional(propagation=Propagation.REQUIRED)
	public void updateAlarmAssignOperatorStat(String oldOperId, 
			long oldAssignTime, boolean oldStatus, String newOperID, long newAssignTime) {
	...
	public void modMonitorFraudTypeStat(Map<String, Object> map) {
		synchronized (object) {
			...
			String readSql = "select count(*) STAT_SIZE from TMS_MONITOR_TXN_FRAUD_STAT t where t.COUNTRYCODE = :COUNTRYCODE " +
					...
					"and t.TXNID = :TXNTYPE and t.TIME = :TIME and t.MODEL_ID = :MODELID and t.DISPOSAL = :DISPOSAL " +
					"and t.FRAUD_TYPE = :FRAUD_TYPE";
