package cn.com.higinet.tms35.manage.tran.service.impl;
...
import cn.com.higinet.tms35.manage.common.DBConstant.TMS_COM_REFFD;
import cn.com.higinet.tms35.manage.common.DBConstant.TMS_COM_REFTAB;
...
public class TransModelServiceImpl implements TransModelService {
			...
			for (Map<String, Object> txndef : txndefs) {
				...
				if (MapUtil.getString(txndef, TMS_COM_TAB.TAB_NAME).equals(txnids[i])) {
					if (i > 0) {
						...
						idPlusName[i] = txnids[i] + "____" + lastname + StaticParameters.SIGNPOST
								+ MapUtil.getString(txndef, TMS_COM_TAB.TAB_DESC);
					} else
	...
	private List<Map<String, Object>> getFartherTranDef(String txnid) {
		...
		sb.append("select " + TMS_COM_TAB.TAB_NAME + "," + TMS_COM_TAB.TAB_DESC + " from " + TMS_COM_TAB.TABLE_NAME
				+ " where " + TMS_COM_TAB.TAB_NAME + " in(" + TransCommon.arr2str(TransCommon.cutToIds(txnid)) + ")");
	...
	public List<Map<String, Object>> getCanRefTable() {
		...
		sb.append("where ");
		sb.append("fd.").append(TMS_COM_FD.TAB_NAME).append("=").append("tab.").append(TMS_COM_TAB.TAB_NAME)
				.append(" and ");
	...
	private void checkRefTabRefed(String ref_id) {
		...
		sb1.append(" from ").append(TMS_COM_REFFD.TABLE_NAME);
		sb1.append(" reffd left join TMS_COM_TAB tab on tab.tab_name=reffd.tab_name where ")
				.append(TMS_COM_REFFD.REF_ID);
		sb1.append(" = ?");

		List<Map<String, Object>> ref_fds = tmsSimpleDao.queryForList(sb1.toString(), Long.parseLong(ref_id));

		if (ref_fds.size() > 0) {
			throw new TmsMgrServiceException("当前表存在" + MapUtil.getString(ref_fds.get(0), "tab_desc") + "引用字段:"
					+ MapUtil.getString(ref_fds.get(0), "REF_NAME"));
		}
	...
	private Map updateRefFdModel(List update) {
		...
		if (!ref_name.equals(ref_name_old)) { // 修改了ref_name,src_id,fd_name,type,而且
			...
			// 2.新值重复
			transDefService.checkSingleParentAndAllSubSame(ref_name_old + "|" + tab_name + "|" + ref_id, ref_name,
					tab_name, false, "reffd", false);
			transDefService.checkSingleParentAndAllSubSame(ref_name_old + "|" + tab_name, ref_name, tab_name, false,
					"fd", false);
		}

		if (!desc.equals(desc_old)) { // 修改了ref_desc
			transDefService.checkSingleParentAndAllSubSame(ref_name_old + "|" + tab_name + "|" + ref_id, desc, tab_name,
					false, "reffd_ref_desc", false);
			transDefService.checkSingleParentAndAllSubSame(ref_name_old + "|" + tab_name, desc, tab_name, false,
					"fd_name", false);
		}
	...
	private String buildAddRefFdModelSql() {
		...
		mdlSb.append(
				" values (:REF_ID,:TAB_NAME,:REF_NAME,:REF_FD_NAME,:SRC_COND,:SRC_EXPR,:SRC_COND_IN,:SRC_EXPR_IN,:STORECOLUMN,:REF_DESC)");
	...
	private String[] tmsNumberType = { "long", "double", "money", "date", "datetime" };
	...
	private Map updateModel(List update) {
		...
		if (!ref_name.equals(ref_name_old) || !src_id.equals(src_id_old) || !fd_name.equals(fd_name_old)
				|| !type.equals(type_old)) { // 修改了ref_name,src_id,fd_name,type,而且
		...
		if (!name.equals(name_old)) { // 修改了ref_name
			// 属性名称新值的重复
			transDefService.checkSingleParentAndAllSubSame(ref_name_old + "|" + tab_name, name, tab_name, false,
					"fd_name", false);
		}
	...
	private void checkRefNameBeRefed(String tab_name, String ref_name, String type) {
		...
		if (ref_fd.size() > 0 || ref_act.size() > 0 || ref_stat.size() > 0 || ref_rule.size() > 0
				|| ref_sw.size() > 0) {
	...
	private String buildAddModelSql() {
		...
		mdlSb.append("values (");
		mdlSb.append(":REF_NAME,:NAME,:TYPE,:CODE,:SRC_ID,:SRC_DEFAULT,:FD_NAME,:GENESISRUL,"
				+ ":DB_TYPE,:DB_LEN,:ORDERBY,:TXN_ORDER,:LINK,"
	...
	public boolean isDuplicateByTransAttrAndRoster(String attrCode) {
		String sql = "select * from " + DBConstant.TMS_MGR_ROSTER + " r where upper(r."
				+ DBConstant.TMS_MGR_ROSTER_ROSTERNAME + ") = ? and " + DBConstant.TMS_MGR_ROSTER_STATUS + " = '1'";
	...
	public List<Map<String, Object>> getTransByTabnameList(Map<String, String> conds) {
		...
		List<Map<String, Object>> list = tmsSimpleDao
				.queryForList("SELECT t.tab_name,t.tab_desc FROM tms_com_tab t " + " START WITH t.tab_name = ?"
						+ " CONNECT BY PRIOR t.parent_tab=t.tab_name", conds.get("txntype").toString());
		...
		for (int i = 0; i < list.size(); i++) {
			...
			if (i == 0) {
				listdb = tmsSimpleDao.queryForList(" SELECT * FROM tms_com_fd d WHERE d.tab_name=? ",
						list.get(i).get("TAB_NAME").toString());
			} else {
				listfor = tmsSimpleDao.queryForList(" SELECT * FROM tms_com_fd d WHERE d.tab_name=? ",
						list.get(i).get("TAB_NAME").toString());
				if (listfor.size() != 0) {
	...
	public Page<Map<String, Object>> getTrandataPage(Map<String, String> conds, List<Map<String, Object>> listdb, boolean isPage) {
		...
		Object suffix = tmsSimpleDao
				.queryForList("SELECT CASE c.is_suffix WHEN 0 THEN '' WHEN 1 THEN '_N' else '_N' END tab_name "
						+ "FROM (SELECT a.is_suffix FROM tms_mgr_iplog a where a.iplog_id=(SELECT max(b.iplog_id) "
						+ "FROM tms_mgr_iplog b where b.operate_result=1)) c")
				.get(0).get("TAB_NAME");
		...
		strsql.append(
				" WHERE traffic.countrycode = region.countrycode AND traffic.regioncode = region.regioncode), '未知') || '-' ||");
