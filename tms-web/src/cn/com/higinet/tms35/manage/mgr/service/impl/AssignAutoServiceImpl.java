...
import cn.com.higinet.tms35.manage.mgr.service.AssignAutoService;
...
public class AssignAutoServiceImpl implements AssignAutoService{
	...
	public Page<Map<String, Object>> dealerInformationPage(Map<String, String> reqs) {
		...
		sql.append("select o.login_name,o.REAL_NAME,o.operator_id,a.as_id,a.as_strategyid,a.starttime,a.endtime,a.is_enable,a.disposals from cmc_operator o,tms_mgr_alarm_autoassign a where o.operator_id=a.dealerid and a.as_strategyid=:AS_STRATEGYID");
		Page<Map<String,Object>> dealerInfoPage = tmpSimpleDao.pageQuery(sql.toString(), reqs, new Order().desc("login_name"));
		for (Map<String, Object> dealerMap : dealerInfoPage.getList()) {
			...
			if(disposals!=null){
				...
				for(int i=0; i<split.length; i++){
					Map<String, Object> List = tmpSimpleDao.retrieve("TMS_COM_DISPOSAL", MapWrap.map("DP_CODE", split[i]).getMap());
	...
	public Page<Map<String, Object>> eventTypePage(Map<String, String> reqs) {
		...
		Page<Map<String,Object>> eventTypePage = tmpSimpleDao.pageQuery(sql.toString(), reqs, new Order().asc("dp_order"));
	...
	public void updateUserAssign(Map<String, String> reqs){
		...
		tmpSimpleDao.executeUpdate(sql.toString(), reqs);
	}
	...
	public void delUserAssign(Map<String,List<Map<String,String>>> batchMap) {
		...
		for(Map<String,String> delMap : delList){
			...
			tmpSimpleDao.delete("TMS_MGR_ALARM_AUTOASSIGN",conds);
			}
		}		
	}
	...
	public Page<Map<String, Object>> getAllUserAssign(Map<String, String> reqs) {
		StringBuffer sql=new StringBuffer();
		sql.append("select o.operator_id,o.login_name,o.REAL_NAME from CMC_OPERATOR o,CMC_OPERATOR_ROLE_REL r where o.operator_id=r.operator_id and o.operator_id not in (select a.dealerid from TMS_MGR_ALARM_AUTOASSIGN a where a.as_strategyid=:TAB_NAME)");
		Page<Map<String, Object>> userAssign = tmpSimpleDao.pageQuery(sql.toString(), reqs, new Order().desc("login_name"));	
		return userAssign;
	}
	...
	public void addUserAssign(Map<String, String> reqs) {
		StringBuffer sql=new StringBuffer();
		String as_id = sequenceService.getSequenceIdToString("SEQ_TMS_ASSIGN_ID",tmpTmsDataSource);
		reqs.put("AS_ID", as_id);
		sql.append("insert into TMS_MGR_ALARM_AUTOASSIGN (AS_ID,AS_STRATEGYID,DEALERID,STARTTIME,ENDTIME,IS_ENABLE,DISPOSALS) values (:AS_ID,:AS_STRATEGYID,:OPERID,:STARTTIME,:ENDTIME,:IS_ENABLE,:DISPOSALS) ");
		tmpSimpleDao.executeUpdate(sql.toString(), reqs);
	}
	...
	public void modStatusUserAssign(Map<String, String> reqs) {
		StringBuffer sql=new StringBuffer();
		Map<String, Object> List = tmpSimpleDao.retrieve("TMS_MGR_ALARM_AUTOASSIGN", MapWrap.map("AS_ID", reqs.get("AS_ID")).getMap());
	...
	public String getAssignSwitchStatus(Map<String, String> reqs) {
		...
		Page<Map<String, Object>> startvalue = tmpSimpleDao.pageQuery(sql.toString(), reqs, new Order().desc("startvalue"));
	...
	public Page<Map<String, Object>> getAllPaySuspendList(Map<String, String> reqs) {
		StringBuffer sql=new StringBuffer();
		sql.append("select u.mobile,u.cusname,u.userid,p.ps_time from TMS_MGR_ALARM_PROCESS p,TMS_RUN_TRAFFICDATA t,TMS_RUN_USER u where p.txn_code=t.txncode and t.userid=u.userid");
		if(!StringUtil.isEmpty(reqs.get("MOBILE")))
			sql.append(" and u.MOBILE like '%'||:MOBILE||'%'");
		if(!StringUtil.isEmpty(reqs.get("CUSNAME")))
			sql.append(" and u.CUSNAME like '%'||:CUSNAME||'%'");
		...
		Page<Map<String,Object>> PaySuspendPage = tmpSimpleDao.pageQuery(sql.toString(), reqs, new Order().asc("ps_time"));
