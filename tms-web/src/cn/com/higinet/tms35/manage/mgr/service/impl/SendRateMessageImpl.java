...
public class SendRateMessageImpl implements SendRateMessage {

	private static Logger log = Logger.getLogger(SendRateMessage.class);
	...
	private void saveReturnInfo(Map<String, Object> transaction,
			String user_id, String resultInfo) {
		log.debug(" 返回的报文："+ resultInfo);
	...
	private void saveTmsUserratelog(String userId, String score, String level,String txnCode,String txnType) {
		//记录用户评级日志
		if(StringUtil.isBlank(userId)||
				StringUtil.isBlank(score)||
				StringUtil.isBlank(level)||
				StringUtil.isBlank(txnCode)||
				StringUtil.isBlank(txnType)){
	...
	{	Map<String,Object> riskResult = null;
		try
		{
			if (resultMessage != null)
			{
				...
				// 截取报文头
				System.arraycopy(bm, StaticParameters.MESSAGE_LEN_LEN, head, 0,
						StaticParameters.HEAD_LEN);
				String headStr = new String(head);
				// 从报文头中截取返回码
				String backCode = headStr.substring(headStr.length() - 8);

				String bodyStr = resultMessage.substring(StaticParameters.MESSAGE_LEN_LEN
						+ StaticParameters.HEAD_LEN);
