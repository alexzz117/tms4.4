...
import cn.com.higinet.tms35.manage.mgr.service.RateService;
import cn.com.higinet.tms35.manage.mgr.service.SendRateMessage;
...
public class RateServiceImpl implements RateService {
	...
	*/
	public List<Map<String, Object>> rateList(Map<String, String> conds) {
		...
		for (Map<String, Object> map : rosterPage) {
			Map<String, Object> tab_data = tmsSimpleDao.retrieve(DBConstant.TMS_COM_TAB.TABLE_NAME, MapWrap.map(DBConstant.TMS_COM_TAB.TXNID, MapUtil.getString(map, "TXNID")).getMap());
	...
	/* (non-Javadoc)
	 * @see cn.com.higinet.tms35.manage.mgr.service.RateService#levelPage(java.util.Map)
	 */
	@Override
	public Page<Map<String,Object>> levelPage(Map<String, String> reqs) {
		...
		sql.append( "SELECT T.USERID USER_ID,T.USERNAME USER_NAME,R.RR_ID,R.RS_ID,R.SCORE,R.RISKLEVEL,R.RATEKIND_ID,R.MODTIME FROM TMS_RUN_USER T LEFT JOIN TMS_MGR_RATERESULT R ON T.USERID=R.RATEKIND_ID where 1=1 and  T.DEALER_FLAG='GENERAL' and T.MEMBER_TYPE='PERSON'");
		...
		if(userType.equals("merchant")){
			...
			merchantSql.append("SELECT T.member_no USER_ID ,T.login_name USER_NAME, R.RR_ID, R.RS_ID,R.SCORE,R.RISKLEVEL, R.RATEKIND_ID,R.MODTIME FROM TMS_Run_Merchants T LEFT JOIN TMS_MGR_RATERESULT R ON T.member_no = R.RATEKIND_ID where 1 = 1");
			   if(isQuery.equals("false")) {
					return null;
				}
				SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
		...
		if(userType.equals("posMerchant")){
			   ...
			   posMerchantSql.append("SELECT T.mrch_no USER_ID,T.MRCH_NAME_C  USER_NAME,R.RR_ID, R.RS_ID,R.SCORE,R.RISKLEVEL,R.RATEKIND_ID,R.MODTIME FROM TMS_RUN_MERCHANT_POS T LEFT JOIN TMS_MGR_RATERESULT R ON T.MRCH_NO = R.RATEKIND_ID  where 1 = 1");
			   if(isQuery.equals("false")) {
					return null;
				}
				SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
	...
	/* (non-Javadoc)
	 * @see cn.com.higinet.tms35.manage.mgr.service.RateService#singleRete(java.util.Map)
	 */
	@Override
	...
	public void allRate(Map<String, Object> reqs,Map<Integer,Map<String,Object>> ser_map) {
		...
		long total = officialJdbcTemplate.queryForLong(countsql);
		if(total == 0 ) {
	...
	public void queryRate(Map<String, Object> reqs,Map<Integer, Map<String, Object>> ser_map) {
		...
		StringBuilder sql = new StringBuilder();
		if(table_name.equals("tms_run_user")){
		sql.append( "SELECT T2.*,R.RR_ID,R.SCORE,R.RISKLEVEL,R.MODTIME FROM (SELECT T.USERID USER_ID,T.USERNAME USERNAME,T1.RS_ID,T1.TXNID,T1.TABLE_NAME,T1.TABLE_PK,T1.LEVEL_SCORE FROM TMS_RUN_USER" 
		...
		if(table_name.equals("tms_run_merchants")){
			sql.append( "SELECT T2.*,R.RR_ID,R.SCORE,R.RISKLEVEL,R.MODTIME FROM (SELECT T.member_no USER_ID,T.login_name login_name,T1.RS_ID,T1.TXNID,T1.TABLE_NAME,T1.TABLE_PK,T1.LEVEL_SCORE FROM TMS_RUN_MERCHANTS" 
		...
		//pos评级
		if(table_name.equals("tms_run_merchant_pos")){
			sql.append( "SELECT T2.*,R.RR_ID,R.SCORE,R.RISKLEVEL,R.MODTIME FROM (SELECT T.mrch_no USER_ID,t.mrch_name_c MRCH_NAME_C,T1.RS_ID,T1.TXNID,T1.TABLE_NAME,T1.TABLE_PK,T1.LEVEL_SCORE FROM TMS_RUN_MERCHANT_POS" 
	...
	*/
	private  String getLevel(String score, String level_score) {
		...
		if ( (Double.valueOf(score) >= tempScore) && 
				(Double.valueOf(score) < Double.valueOf("100"))) {
			return levels[0].split(",")[1];
		}
	...
	public List<Map<String, Object>> getTransHitRuleList(String txnCode, String txnType) {
		...
		if(totalTrigRules!=trigList.size()){
			ruleTempSql = "select RULE_ID, RULE_NAME, RULE_DESC, RULE_COND, RULE_COND_IN, " + "RULE_SHORTDESC, EVAL_TYPE, DISPOSAL from TMS_COM_RULE_TEMP where RULE_TXN in (" + TransCommon.arr2str(TransCommon.cutToIds(txnType)) + ")";
