...
public class dao_serv_live
{
	...
	public dao_serv_live(db_server serv_info)
	{
		...
		insert = new batch_stmt_jdbc(ds, insertSql + ((SqlMap) bean.get("tmsSqlMap")).getSql("tms.dao.merge.serv_live_insert")
				, new int[] { Types.VARCHAR, Types.INTEGER, Types.INTEGER, Types.INTEGER });
		
		update = new batch_stmt_jdbc(ds, ((SqlMap) bean.get("tmsSqlMap")).getSql("tms.dao.merge.serv_live_update")
