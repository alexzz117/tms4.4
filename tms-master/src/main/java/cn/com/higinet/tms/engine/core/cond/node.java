...
import cn.com.higinet.tms.engine.core.cond.func_map.map_function;
import cn.com.higinet.tms.engine.core.cond.parser.cond_parser;
import cn.com.higinet.tms.engine.core.cond.parser.tms_exception_lexer;
...
public class node {
	...
	public Object exec(run_env rd, Stack stack) {
		if (m_op == op.const_)
			return value;

		if (fn_index >= 0) {
			func f = func_map.get(fn_index);
			if (this.m_op == op.and_) {
				return func_map.is_true(child[0].exec(rd, stack)) && func_map.is_true(child[1].exec(rd, stack)) ? func_map.TRUE : func_map.FALSE;
			} else if (this.m_op == op.or_) {
				return func_map.is_true(child[0].exec(rd, stack)) || func_map.is_true(child[1].exec(rd, stack)) ? func_map.TRUE : func_map.FALSE;
			} else {
				stack.push(rd);
				if (child != null) {
					for (int i = 0, len = child.length; i < len; i++)
						stack.push(child[i].exec(rd, stack));
				}
				int n = 1 + stack.pop(child == null ? 0 : child.length);
				try {
					Object ret = f.exec(stack.buf(), n);
					stack.pop(1);
					return ret;
				} catch (RuntimeException e) {
					StringBuffer sb = new StringBuffer();
					sb.append("交易流水号: " + rd.get_txn_code() + ", 执行表达式: " + toString() + ",函数Class:" + (f == null ? "null" : f.getClass()) + ",函数index:" + fn_index + ", 函数参数:[");
					for (int i = n; i <= stack.pos; i++) {
						sb.append(stack.buff[i]).append(",");
					}
					sb.append("], 报错.");
		...
		if (type == op.table_)
			return new Long(this.n1);

		if (name == null) {
			String s = "节点没有名称(name)属性:" + this;
		...
		if (name.charAt(0) == 'S') {
			if (child == null || child[0].m_op != op.stat_param_) {
				return rd.get_stat_value(n1, null);
			}

			return rd.get_stat_value(n1, rd.get_fd_value(child[0].n1));
		}

		if (name.charAt(0) == 'F')
			return rd.get_fd_value(n1);

		if (name.charAt(0) == 'C') {
			return rd.get_chan_value(n1);
		}

		throw new tms_exception("无法处理的节点：", this.toString());
	}

	public Object exec(run_env rd) {
		Stack stack = new Stack();
		return exec(rd, stack);
	}
