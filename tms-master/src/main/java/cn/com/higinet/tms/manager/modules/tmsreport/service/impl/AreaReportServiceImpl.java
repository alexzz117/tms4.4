...
public class AreaReportServiceImpl extends ApplicationObjectSupport implements AreaReportService {
	...
	private List<Map<String, Object>> convertTxnList( List<Map<String, Object>> reportList, String id, String name ) {
		...
		for( Map<String, Object> map : list ) {
			...
			for( Map<String, Object> code : pslist ) {
				...
				map.put( key + ReportConstant.REPORT_PS_RATE, sb.append( MapUtil.getInteger( map, key + ReportConstant.REPORT_PS_NUM ) ).append( "(" ).append( getRate( MapUtil.getInteger( map, key + ReportConstant.REPORT_PS_NUM ), txnnumber ) ).append( ")" ).toString() );
				sb.setLength( 0 );
				map.put( key + ReportConstant.REPORT_PS_RATE_FD, sb.append( MapUtil.getInteger( map, key + ReportConstant.REPORT_PS_NUM_FD ) ).append( "(" ).append( getRate( MapUtil.getInteger( map, key + ReportConstant.REPORT_PS_NUM_FD ), txnnumber ) ).append( ")" ).toString() );
				sb.setLength( 0 );
				map.put( key + ReportConstant.REPORT_PS_RATE_NFD, sb.append( MapUtil.getInteger( map, key + ReportConstant.REPORT_PS_NUM_NFD ) ).append( "(" ).append( getRate( MapUtil.getInteger( map, key + ReportConstant.REPORT_PS_NUM_NFD ), txnnumber ) ).append( ")" ).toString() );
				sb.setLength( 0 );
			}
		}
		return list;
	...
	public String getRate( int numerator, int denominator ) {
		...
		BigDecimal cc = new BigDecimal( (Float.intBitsToFloat( numerator ) * 100) / Float.intBitsToFloat( denominator ) ).setScale( 2, BigDecimal.ROUND_HALF_UP );
	...
	private String getAreaReportSQL( Map<String, String> conds, String tmsSqlName, String extSql, String groupBySql, String relSql, Map<String, Object> sql_conds ) {
		...
		sql.append( " " ).append( groupBySql ).append( tmsSqlMap.getSql( extSql ) ).append( " " ).append( condition ).append( " " ).append( groupBySql ).append( " " ).append( relSql );
