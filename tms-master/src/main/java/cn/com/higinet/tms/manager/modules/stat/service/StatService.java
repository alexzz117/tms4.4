...
public class StatService {
	...
	private DataSourceTransactionManager onlineTransactionManager;
	...
	public Map<String, Object> saveStat( Map<String, List<Map<String, ?>>> formList ) {
		...
		if( validNList != null && validNList.size() > 0 ) {
			...
			for( Map<String, Object> map : validNList ) {
				...
				String stat_desc = MapUtil.getString( map, DBConstant.TMS_COM_STAT_STAT_DESC );
				// 缓存中查找引用
				if( commonCheckService.find_ref_valid_stat( txnId, stat_name ) ) throw new TmsMgrServiceException( "[" + stat_desc + "]被引用未授权，不能停用" );
			}
	...
	*/
	...
	private Map<String, Object> createStat( Map<String, Object> input ) {
		...
		if( result_cond.length() > 0 ) stat.put( DBConstant.TMS_COM_STAT_RESULT_COND, StringUtil.parseToLong( result_cond ) );

		String counuint = MapUtil.getString( input, DBConstant.TMS_COM_STAT_COUNUNIT );
		if( counuint.length() > 0 ) stat.put( DBConstant.TMS_COM_STAT_COUNUNIT, StringUtil.parseToLong( counuint ) );
		...
		if( unresult.length() > 0 ) stat.put( DBConstant.TMS_COM_STAT_STAT_UNRESULT, StringUtil.parseToLong( unresult ) );

		String continues = MapUtil.getString( input, DBConstant.TMS_COM_STAT_CONTINUES );
		if( continues.length() > 0 ) stat.put( DBConstant.TMS_COM_STAT_CONTINUES, StringUtil.parseToLong( continues ) );
