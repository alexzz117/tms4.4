...
import cn.com.higinet.tms.manager.modules.query.model.Filed;
import cn.com.higinet.tms.manager.modules.query.model.QueryCond;
import cn.com.higinet.tms.manager.modules.query.model.QueryJsonVO;
import cn.com.higinet.tms.manager.modules.query.model.QueryResult;
import cn.com.higinet.tms.manager.modules.query.model.QueryScript;
import cn.com.higinet.tms.manager.modules.query.model.Statement;
import cn.com.higinet.tms.manager.modules.query.model.Table;
import cn.com.higinet.tms.manager.modules.query.model.QueryCond.Cond;
import cn.com.higinet.tms.manager.modules.query.model.QueryResult.Column;
...
public class QueryJsonUtil {
	...
	public static QueryJsonVO jsonToVo(String json) throws Exception {
		...
		try {
			if (CmcStringUtil.isBlank(json)) return null;
			...
			if(CmcStringUtil.isBlank(queryType) || (!CmcStringUtil.isBlank(queryType)
			...
			if(jsonVO.getStatement().getType() < 3) {
				...
				if(CmcStringUtil.isBlank(spellType) || (!CmcStringUtil.isBlank(spellType)
	...
	private static Statement makeStatement(Map<String, Object> map) throws Exception{
		...
		if(CmcStringUtil.isBlank(type) || (!CmcStringUtil.isBlank(type)
	...
	private static List<Table> makeTables(List<Map<String, Object>> tables) throws Exception {
		...
		for(Map<String, Object> tmap : tables) {
			...
			if(CmcStringUtil.isBlank(tabName) || CmcStringUtil.isBlank(tabAlias)) {
	...
	private static List<Filed> makeFileds(List<Map<String, Object>> fileds) throws Exception {
		...
		for(Map<String, Object> fmap : fileds) {
			...
			if(tmsType.equalsIgnoreCase("code") && CmcStringUtil.isBlank(code)) {
			...
			filed.setDb_len(CmcStringUtil.isBlank(dbLen) ? 0 : Integer.parseInt(dbLen));
			filed.setCode(code);
	...
	private static QueryCond makeQueryCond(Map<String, Object> queryCondMap) throws Exception {
		...
		queryConds.setLine_conds(CmcStringUtil.isBlank(lineConds) ? 4 : Integer.parseInt(lineConds));
		String qfromDisplay = MapUtil.getString(queryCondMap, QUERY_CUSTOM_QUERY_COND_QFROM_DISPLAY);
		queryConds.setQfrom_display(CmcStringUtil.isBlank(qfromDisplay) ? false : Boolean.valueOf(qfromDisplay));
		...
		if(MapUtil.getObject(queryCondMap, QUERY_CUSTOM_QUERY_COND_CONDS) instanceof List) {
			...
			queryConds.setConds(new LinkedList<Cond>() {{
				for(Map<String, Object> condMap : condsList) {
					...
					if(CmcStringUtil.isBlank(title)) {
					...
					if(MapUtil.getObject(condMap, QUERY_CUSTOM_QUERY_COND_CONDS_SUBCONDS) instanceof List) {
						...
						cond.setSubconds(new LinkedList() {{
							for(Map<String, Object> subCondMap : subCondsList) {
								...
								if(CmcStringUtil.isBlank(recFiled)) {
								...
								subCond.setName(CmcStringUtil.isBlank(name) ? MapUtil.getString(subCondMap, QUERY_CUSTOM_QUERY_COND_CONDS_SUBCONDS_NAME) : name);
	...
	private static QueryResult makeQueryResult(Map<String, Object> queryResultMap) throws Exception {
		...
		if(MapUtil.getObject(queryResultMap, QUERY_CUSTOM_QUERY_RESULT_COLUMNS) instanceof List) {
			...
			queryResult.setColumns(new LinkedList<Column>() {{
				for(Map<String, Object> columnMap : columns) {
					...
					column.setWidth(CmcStringUtil.isBlank(width) ? 0 : Integer.parseInt(width));
					String hide = MapUtil.getString(columnMap, QUERY_CUSTOM_QUERY_RESULT_COLUMNS_HIDE);
					column.setHide(CmcStringUtil.isBlank(hide) ? false : (Boolean.valueOf(hide)));
					...
					column.setOpen_link(CmcStringUtil.isBlank(openLink) ? 0 : Integer.parseInt(openLink));
		...
		queryResult.setDistinct_display(CmcStringUtil.isBlank(distinctDisplay) ? false : Boolean.valueOf(distinctDisplay));
		queryResult.setDisplay_type(MapUtil.getString(queryResultMap, QUERY_CUSTOM_QUERY_RESULT_DISPLAY_TYPE));
		queryResult.setPaging(CmcStringUtil.isBlank(pageing) ? false : Boolean.valueOf(pageing));
		queryResult.setPage_size(CmcStringUtil.isBlank(pageSize) ? 10 : Integer.parseInt(pageSize));
		return queryResult;
	}
