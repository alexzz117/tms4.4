...
public class CommonCheckService {
	...
	public void findRefField( String tab_name, String ref_name, List<db_fd> ref_fd, List<db_rule_action> ref_act, List<db_stat> ref_stat, List<db_rule> ref_rule,
			List<db_strategy> ref_sw, DataSource mDs ) {
	...
	public static void checkCond( Map map, String... strings ) throws IllegalArgumentException {
		...
		if( strings.length != 3 || !map.containsKey( strings[0] ) || !map.containsKey( strings[1] ) || !map.containsKey( strings[2] ) ) {
	...
	public boolean find_ref_roster( String roster_name, List<db_stat> ref_stat, List<db_rule> ref_rule, List<db_strategy> ref_sw, List<db_rule_action> ref_act ) {
	...
	public boolean find_ref_stat( String txn_name, String stat_name, List<db_stat> ref_stat, List<db_rule> ref_rule, List<db_strategy> ref_sw, List<db_rule_action> ref_ac,
	...
	private boolean isEnableRef( List<db_stat> ref_stat, List<db_rule> ref_rule, List<db_rule_action> ref_act, List<db_strategy> ref_sw, List<db_process> ref_ps ) {
		...
		if( ref_act != null ) {
			for( db_rule_action db_action : ref_act ) {
				if( db_action.is_enable ) return true;
			}
		}
		if( ref_stat != null ) {
			for( db_stat db_stat : ref_stat ) {
				if( db_stat.is_valid == 1 ) return true;
			}
		}
		if( ref_sw != null ) {
			for( db_strategy db_sw : ref_sw ) {
				if( db_sw.st_enable ) return true;
			}
		}
		if( ref_rule != null ) {
			for( db_rule db_rule : ref_rule ) {
				if( db_rule.is_enable ) return true;
			}
		}
		if( ref_ps != null ) {
			for( db_process db_process : ref_ps ) {
				if( db_process.ps_enable ) return true;
			}
		}
		return false;
	}
