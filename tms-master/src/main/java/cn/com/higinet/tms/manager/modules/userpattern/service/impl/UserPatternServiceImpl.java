...
public class UserPatternServiceImpl implements UserPatternService {
	private static final Logger log = LoggerFactory.getLogger( UserPatternServiceImpl.class );
	...
	private String[] map2UserPattern( Map<String, List<String[]>> m ) {
		...
		if( MapUtil.isEmpty( m ) ) return null;
		...
		for( Entry<String, List<String[]>> entry : enSet ) {
			...
			for( String[] s : oneStat ) {
				...
				userPatter.append( s[StaticParameters.STATID_INDEX] ).append( split_1 ).append( s[StaticParameters.UPVALUE_INDEX] ).append( split_1 ).append( s[StaticParameters.STARTDATE_INDEX] ).append( split_1 ).append( s[StaticParameters.ENDDATE_INDEX] ).append( split_1 ).append( s[StaticParameters.UPID_INDEX] ).append( "|" );
			}
			userPatter.append( "\n" );
		}
	...
	private Map<String, List<String[]>> userPattern2Map( String us ) {
		...
		if( StringUtil.isEmpty( us ) ) return r;
	...
	private String[] checkDuplicateUserPattern( Map<String, Object> map, Map<String, List<String[]>> upMap, String statid ) {
		...
		for( String[] s : list ) {
			...
			if( stat_fn.equals( "rang_bin_dist" ) || stat_fn.equals( "bin_dist" ) ) {
				...
				if( in_id.compareTo( db_id ) != 0 && (in_startDate.compareTo( db_startDate ) <= 0 && (in_endDate.compareTo( db_startDate ) >= 0 || StringUtil.isEmpty( in_endDate )) || (in_startDate.compareTo( db_startDate ) >= 0 && (in_startDate.compareTo( db_endDate ) <= 0 || StringUtil.isEmpty( db_endDate )))) ) {
		...
		String[] a = {
				statid, in_value, in_startDate, in_endDate, in_id
		};
	...
	public void synUserPatternData( String userId, String statId, String patternId, String operate, boolean isPass ) {
		...
		if( "add".equals( operate ) ) {
			...
			if( isPass ) {
				...
				if( s_u_l_t.size() == 0 ) up_map_t.remove( statId );
			...
			if( isPass ) {
				...
				if( s_u_l_o.size() == 0 ) up_map_o.remove( statId );
