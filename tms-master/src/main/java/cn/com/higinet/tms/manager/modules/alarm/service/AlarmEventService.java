...
public class AlarmEventService {
	...
	public List<Map<String, Object>> getAlarmProcessInfoList( Map<String, String> cond ) {
		...
		for( Map<String, Object> map : list ) {
			...
			if( short_act.length() == 0 ) continue;
			String short_act_name = "";
			String[] acts = short_act.split( "," );
			for( String act : acts ) {
				if( short_act_name.length() > 0 ) short_act_name += ",";
	...
	private void oneAlarmProcess( Map<String, String> cond, HttpServletRequest request ) {
		...
		monitorStatService.updateAlarmProcessOperatorStat( txnOperId, MapUtil.getLong( transMap, "ASSIGNTIME" ), CmcStringUtil.isBlank( MapUtil.getString( transMap, "ASSIGNID" ) ),
				true );
	...
	private void addShortAction( Map<String, String> cond ) {
		...
		for( Map db_ac : short_ac_list ) {
			...
			if( !isFind ) delOneAlarmProcessAction( db_ac );
		}
	}
	...
	private Map<String, Object> buildTransactionMap( Map<String, Object> transMap ) {
		...
		String[] fields = new String[] {
				"TXNCODE", "CHANCODE", "TXNID", "USERID", "TXNSTATUS", "PSSTATUS"
		};
	...
	public void alarmAssign( Map<String, String> cond, HttpServletRequest request ) {
		...
		monitorStatService.updateAlarmAssignOperatorStat( MapUtil.getString( transMap, "OPERID" ), MapUtil.getLong( transMap, "ASSIGNTIME" ), status,
				MapUtil.getString( map, "OPERID" ), currTime );
		updateTransProcessInfo( map );
	}
	...
	*/
	public Page<Map<String, Object>> getAlarmHisActions( Map<String, String> reqs ) {
		String sql = "select t.userid,u.cusname,t.txncode,p.fraud_type,p.ps_info,p.short_action,p.ps_time,t.OPERID from tms_run_trafficdata t"
				...
				+ " inner join tms_mgr_alarm_process  p on p.txn_code=t.txncode and p.ps_time=a.ps_time" + " where t.OPERID = :OPERID and t.userid = :userid";
