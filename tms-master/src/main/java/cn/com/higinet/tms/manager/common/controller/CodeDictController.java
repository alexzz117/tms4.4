...
public class CodeDictController {
	...
	public void setCodeDictService(CodeDictService codeDictService) {
	...
	public Model categoryAddAction(@RequestParam Map<String,String> reqs){
		Model model = new Model();
	...
	public Model getCategoreyAction(@RequestParam String categoryId){
		Model model = new Model();
	...
	public Model updateCategoreyAction(@RequestParam Map<String,String> regs){
		Model model = new Model();
		String runmode = codeDict.getCode("common.model", "runmode");
		if(runmode != null && "development".equals(runmode)){
	...
	public Model codeListAction(@RequestParam Map<String, String> reqs){
		Model model = new Model();
	...
	public Model codeAddAction(@RequestParam Map<String,String> reqs){
		Model model = new Model();
		String runmode = codeDict.getCode("common.model", "runmode");
		if(runmode != null && "development".equals(runmode)){
	...
	public Model delCodeAction(@RequestParam("codeId") String [] codeId ){
		Model model = new Model();
		String runmode = codeDict.getCode("common.model", "runmode");
		if(runmode != null && "development".equals(runmode)){
			if(codeId!=null && codeId.length>0){
				codeDictService.deleteCode(codeId[0]);
			}
		}else{
			model.addError("error.cmc.func.runmode.msg");
		}
	...
	public Model getCodeAction(@RequestParam String codeId){
		Model model = new Model();
	...
	public Model updatecodeAction(@RequestParam Map<String,String> regs){
		Model model = new Model();
		String runmode = codeDict.getCode("common.model", "runmode");
		if(runmode != null && "development".equals(runmode)){
			Map <String ,Object> code = codeDictService.getCode(regs.get("code_id"));
			code.put("CATEGORY_ID", regs.get("category_id"));
			code.put("CODE_KEY", regs.get("code_key"));
			code.put("CODE_VALUE", regs.get("code_value"));
			code.put("ONUM", regs.get("onum"));
			code.put("INFO", regs.get("info"));
