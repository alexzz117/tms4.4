...
public class RuleService {
	...
	public Map<String, Object> saveRule( Map<String, List<Map<String, ?>>> formList ) {
		...
		if( delList != null && delList.size() > 0 ) {
			for( Map<String, Object> map : delList ) {
				...
				long t_strategy_rule = dynamicSimpleDao.count( sql, MapUtil.getString( map, DBConstant.TMS_COM_RULE_RULE_ID ) );
	...
	private void checkDuplicateRuleName( Map<String, Object> cond ) {
		...
		List<Map<String, Object>> ruleList = dynamicSimpleDao.queryForList( sql, txnIdsMap );
	...
	private void deleteRule( Map<String, Object> map ) {
		dynamicSimpleDao.delete( "TMS_COM_RULE", MapWrap.map( DBConstant.TMS_COM_RULE_RULE_ID, MapUtil.getLong( map, DBConstant.TMS_COM_RULE_RULE_ID ) ).getMap() );
		dynamicSimpleDao.delete( "TMS_COM_RULE_ACTION", MapWrap.map( DBConstant.TMS_COM_RULE_ACTION_RULE_ID, MapUtil.getLong( map, DBConstant.TMS_COM_RULE_RULE_ID ) ).getMap() );
	}
					...
					for( Map<String, Object> officialRelationMap : officialRelationList ) {
						if( MapUtil.getString( tempRelationMap, DBConstant.TMS_COM_RULERELATION_RULEREL_TXN ).equals( MapUtil.getString( officialRelationMap, DBConstant.TMS_COM_TAB.TAB_NAME ) ) ) {
							...
							if( StringUtil.isEmpty( MapUtil.getString( officialRelationMap, DBConstant.TMS_COM_RULERELATION_RULEREL_ID ) ) ) {
								...
								List<Map<String, Object>> parentJsonList = dynamicSimpleDao.queryForList( sql1, txnIdsMap );
			...
			if( result ) { // 审核通过
				...
				for( Map<String, Object> reMap : tempRelationList ) {
					if( StringUtil.isNotEmpty( MapUtil.getString( reMap, DBConstant.TMS_COM_RULERELATION_RULEREL_ID ) ) ) {
						...
						if( directionalList != null ) directionalList.remove( MapUtil.getString( templineMap, DBConstant.TMS_COM_RULE_CHILD_CHILD_ID ) );
		...
		if( operate.equals( "del" ) ) { // 删除连接线规则审核
			if( result ) { // 通过
				...
				for( Map<String, Object> offMap : officialRelationList ) {
					...
					List<String> directionalList = (ArrayList<String>) MapUtil.getObject( ruleMap, "DIRECTIONAL" );
					if( directionalList != null ) directionalList.remove( MapUtil.getString( officialLineMap, DBConstant.TMS_COM_RULE_CHILD_CHILD_ID ) );
				...
				for( Map<String, Object> reMap : tempRelationList ) {
					...
					Map<String, Object> ruleMap = MapUtil.getMap( jsonMap, MapUtil.getString( officialLineMap, DBConstant.TMS_COM_RULE_CHILD_RULE_ID ) );
					List<String> directionalList = (ArrayList<String>) MapUtil.getObject( ruleMap, "DIRECTIONAL" );
					if( directionalList != null ) directionalList.add( MapUtil.getString( officialLineMap, DBConstant.TMS_COM_RULE_CHILD_CHILD_ID ) );
	...
	public void checkRef( String txn_id, String rule_name, String ruleShortdesc, String oper ) {
		...
		if( rule_name == null || rule_name.length() == 0 ) return;

		// 校验能否被引用
		// 初始化缓存
		commonCheckService.initCache( offlineDataSource );
		if( "del".equals( oper ) ) {
			if( commonCheckService.find_ref_rule( txn_id, rule_name ) ) throw new TmsMgrServiceException( "[" + ruleShortdesc + "]被引用，不能删除" );
		}
		else {
			if( commonCheckService.find_ref_valid_rule( txn_id, rule_name ) ) throw new TmsMgrServiceException( "[" + ruleShortdesc + "]被引用，不能停用" );
		}

		// 初始化缓存
		commonCheckService.initCache( onlineDataSource );
		if( "del".equals( oper ) ) {
			if( commonCheckService.find_ref_rule( txn_id, rule_name ) ) throw new TmsMgrServiceException( "[" + ruleShortdesc + "]被未授权，不能删除" );
		}
		else {
			if( commonCheckService.find_ref_valid_rule( txn_id, rule_name ) ) throw new TmsMgrServiceException( "[" + ruleShortdesc + "]被未授权，不能停用" );
		}
	}
	...
	public void batchCheckRef( String[] rule_ids, String oper ) {
		...
		for( String rule_id : rule_ids ) {
			...
			if( "del".equals( oper ) ) {
				if( commonCheckService.find_ref_rule( MapUtil.getString( ruleInfo, DBConstant.TMS_COM_RULE_RULE_TXN ), MapUtil.getString( ruleInfo, DBConstant.TMS_COM_RULE_RULE_NAME ) ) ) throw new TmsMgrServiceException( "[" + MapUtil.getString( ruleInfo, DBConstant.TMS_COM_RULE_RULE_SHORTDESC ) + "]被引用，不能删除" );
			}
			else {
				if( commonCheckService.find_ref_valid_rule( MapUtil.getString( ruleInfo, DBConstant.TMS_COM_RULE_RULE_TXN ), MapUtil.getString( ruleInfo, DBConstant.TMS_COM_RULE_RULE_NAME ) ) ) throw new TmsMgrServiceException( "[" + MapUtil.getString( ruleInfo, DBConstant.TMS_COM_RULE_RULE_SHORTDESC ) + "]被引用，不能停用" );
			}
		}
		...
		for( String rule_id : rule_ids ) {
			...
			if( "del".equals( oper ) ) {
				if( commonCheckService.find_ref_rule( MapUtil.getString( ruleInfo, DBConstant.TMS_COM_RULE_RULE_TXN ), MapUtil.getString( ruleInfo, DBConstant.TMS_COM_RULE_RULE_NAME ) ) ) throw new TmsMgrServiceException( "[" + MapUtil.getString( ruleInfo, DBConstant.TMS_COM_RULE_RULE_SHORTDESC ) + "]被引用未授权，不能删除" );
			}
			else {
				if( commonCheckService.find_ref_valid_rule( MapUtil.getString( ruleInfo, DBConstant.TMS_COM_RULE_RULE_TXN ), MapUtil.getString( ruleInfo, DBConstant.TMS_COM_RULE_RULE_NAME ) ) ) throw new TmsMgrServiceException( "[" + MapUtil.getString( ruleInfo, DBConstant.TMS_COM_RULE_RULE_SHORTDESC ) + "]被引用未授权，不能停用" );
			}
		}
	}
}
