...
import cn.com.higinet.tms.engine.comm.MapUtil;
import cn.com.higinet.tms.engine.comm.Message;
import cn.com.higinet.tms.engine.comm.NotErrorException;
import cn.com.higinet.tms.engine.comm.StaticParameters;
import cn.com.higinet.tms.engine.comm.TransNotFoundException;
...
import cn.com.higinet.tms.engine.core.cache.db_action;
...
import cn.com.higinet.tms.engine.service.io_env;
...
//	private db_session m_session;
	...
	private void init_(dao_combin m_dc, counter counterEval) throws Exception {
		...
		if (Stringz.isNotEmpty(txn_code)) {
			if (this.is_confirm()) {
				run_txn_values rf = null;//trafficdata.getById(txn_code, this); //此处改由持久化接口实现，4.3版本流水集中缓存修改  2017-09-05 王兴
				if (!rf.not_empty()) {
					// 查不到对应的风险评估数据
				...
				run_txn_values rf = null;//trafficdata.getCacheById(txn_code, this); //此处改由持久化接口实现，4.3版本流水集中缓存修改  2017-09-05 王兴
