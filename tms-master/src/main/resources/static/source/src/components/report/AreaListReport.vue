  ...
  <div>
    <el-row>
      <el-collapse v-model="activeNames" @change="handleChange">
        <el-collapse-item title="地区报警信息查询" name="search">
        ...
        <el-collapse-item title="地区报警信息展示图" name="chart">
          <div>
        ...
        <el-collapse-item title="地区报警信息列表" name="table">
          <template slot="title">
            <div style="margin:10px;text-align: left ">
              <span style="margin-right: 10px">地区报警信息列表</span>
  ...
  export default {
    ...
    methods: {
        ...
        for (var i = 0; i < g_list.length; i++) {
          if (op_x_data.length >= cf_tops) {
            ...
            if ((g_index) % 2 === 0) {
              ...
              op_x_data.push(g_list[i]['regionname'])
            }
      ...
      exportFunc () {
        ...
        console.info(params)
        let url = '/report/area/export?' + util.serializeObj(params)
        ajax.syncGet({
          url: url
        })
      ...
      printFunc () {
        ...
        option.tablebBox = {
          tableTitle: '地区报警信息列表', // 表格标题
          columns: [],
          tableData: []        // 表格数据
        }
        let tableColumns = this.tableColumns
        let columns = [{name: '地区名称', dataIndex: 'regionname'},
