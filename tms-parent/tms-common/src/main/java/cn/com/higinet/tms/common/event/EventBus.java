 ...
 * 目前提供两种事件总线的实现，一个实现是内部单线程调度的基于ringbuffer实现的{@link cn.com.higinet.tms.common.event.impl.DisruptorEventBus}，
 * 另外一个实现是基于单线程的{@link cn.com.higinet.tms.common.event.impl.SimpleEventBus}。下面详细介绍这两种事件总线的特性：<br/>
 * 1、DisruptorEventBus基于Disruptor高并发框架实现的跨线程通信事件总线。<br/>
 * &nbsp;&nbsp;这个总线是基于Disruptor框架封装的，内部有一个ringbuffer缓存与单线程调度器{@link cn.com.higinet.tms.common.event.impl.DisruptorEventProcessor},
 * 单线程调度器持续不断的从ringbuffer中抽取event事件，并且按照DisruptorEventBus设置的EventDispatcher分发器，分发到订阅了event关联topic的{@link cn.com.higinet.tms.common.event.EventChannel}中去。
 ...
 * }
 * <br/>
 * 由于DisruptorEventBus内部是单线程提交，因此搭配{@link cn.com.higinet.tms.common.event.impl.ConcurrentEventChannel}使用的时候要注意，ConcurrentEventChannel
